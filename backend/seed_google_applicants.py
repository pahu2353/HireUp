"""Seed 40 generic applicants to Google's Software Engineer - New Grad position."""
import json
import sys
...
GOOGLE_NEW_GRAD_JOB = {
    ...
    "description": (
        "Design, develop, and maintain software that runs Google's massive-scale systems. "
        "Work on products that impact billions of users worldwide."
    ),
...
def get_or_create_new_grad_job_id(company_id: str) -> str:
    with database.get_conn() as conn:
        row = conn.execute(
            """
            SELECT id FROM jobs
            ...
            """,
            (company_id, GOOGLE_NEW_GRAD_JOB["title"]),
        ).fetchone()

    if row:
        print(f"Found existing job: {GOOGLE_NEW_GRAD_JOB['title']}")
        return row["id"]

    job_id = database.create_job(
        company_id=company_id,
        title=GOOGLE_NEW_GRAD_JOB["title"],
        description=GOOGLE_NEW_GRAD_JOB["description"],
        skills=json.dumps(GOOGLE_NEW_GRAD_JOB["skills"]),
        location=GOOGLE_NEW_GRAD_JOB["location"],
        salary_range=GOOGLE_NEW_GRAD_JOB["salary_range"],
    )
    print(f"Created job: {GOOGLE_NEW_GRAD_JOB['title']}")
    return job_id


def seed_google_applicants() -> None:
    database.init_db()
    ...
    job_id = get_or_create_new_grad_job_id(company_id)
    ...
    for i in range(1, NUM_APPLICANTS + 1):
        ...
        email = f"google.applicant{i:02d}@hireup.dev"
        ...
        if user:
            ...
            created = database.create_user(
                ...
                interests=json.dumps(["Software Engineering", "Distributed Systems"]),
            )
            if not created:
